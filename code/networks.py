...
class NestedUNet(nn.Module):
    def __init__(self, input_channels=3, num_classes=10, deep_supervision=False, **kwargs):
        super().__init__()
...
class ReLayNet(BasicUNet):
    def __init__(self, in_channels=3, num_classes=10, features=64, kernel=(7,3)):
        ...
        self.encoder1 = ReLayNet._block(in_channels, features, name="enc1", conv_kernel=kernel, conv_pad=conv_pad, repetitions=1)
        self.encoder2 = ReLayNet._block(features, features, name="enc2", conv_kernel=kernel, conv_pad=conv_pad, repetitions=1)
        self.encoder3 = ReLayNet._block(features, features, name="enc3", conv_kernel=kernel, conv_pad=conv_pad, repetitions=1)

        self.bottleneck = ReLayNet._block(features, features, name="bottleneck", conv_kernel=kernel, conv_pad=conv_pad, repetitions=1)

        self.decoder3 = ReLayNet._block(features * 2, features, name="dec3", conv_kernel=kernel, conv_pad=conv_pad, repetitions=1)
        self.decoder2 = ReLayNet._block(features * 2, features, name="dec2", conv_kernel=kernel, conv_pad=conv_pad, repetitions=1)
        self.decoder1 = ReLayNet._block(features * 2, features, name="dec1", conv_kernel=kernel, conv_pad=conv_pad, repetitions=1)
        self.softmax = nn.Softmax2d()
    ...
    def forward(self,x):
        enc1, pool1, indices1, enc2, pool2, indices2, enc3, pool3, indices3, bottleneck = ReLayNet.forward_encoder(self, x)
        dec1=ReLayNet.forward_decoder(self, enc1, pool1, indices1, enc2, pool2, indices2, enc3, pool3, indices3, bottleneck)
        return self.softmax(self.conv(dec1))
...
class LFUNet(BasicUNet):
    def __init__(self, in_channels=3, num_classes=10, features=64, kernel=(7,3)):
        ...
        self.convdil4 = nn.Conv2d(features * 2, int(features / 2), kernel_size=kernel, padding=paddil4, bias=False, dilation=dilation4)
        self.convdil6 = nn.Conv2d(features * 2, int(features / 2), kernel_size=kernel, padding=paddil6, bias=False, dilation=dilation6)
        self.convdil8 = nn.Conv2d(features * 2, int(features / 2), kernel_size=kernel, padding=paddil8, bias=False, dilation=dilation8)
