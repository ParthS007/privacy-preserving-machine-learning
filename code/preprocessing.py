...
class DataPreprocessor:
    ...
    def __init__(
        ...
        data_path,
        dest_path,
        slicing: Callable[[torch.Tensor], list],
        is_mat=False,
        labelled_dataset=True,
    ):
        ...
        self.img_files = [
            ...
            if e.is_file() and self.ft_mat[is_mat].lstrip(".") == e.name.split(".")[-1]
        ]
        assert (
            len(self.img_files) > 0
        ), f"no such {self.ft_mat[is_mat]} file types found in {data_path}"
        self.dest_path = dest_path
        assert (
            data_path != dest_path
        ), "Please provide a dest_path different from data_path"
        ...
        for f in self.img_files:
            ...
            if labelled_dataset:
                ...
                assert self.img_slice_nums[f] == len(slicing(mask)), (
                    f"num slices of image: {f}"
    ...
    def preprocess(self):
        ...
        for img_num in range(len(self.img_files)):
            print(
                f"pre-processing img {img_num +1}/{len(self.img_files)}:{self.img_files[img_num]}"
            )
            split_path = ""
            subject_num = int(
                self.img_files[img_num].split("/")[-1].split(".")[0].split("_")[1]
            )
            if subject_num < 7:
                split_path = "train"
...
class DataPreprocessorUMN:
    ...
    def __init__(
        ...
        for i in range(29):
            ...
            assert self.img_slice_nums[str(i)] == len(slicing(mask)), (
                f"num slices of image: {f}"
                + "  must equal the num slices of corresponding mask"
            )
        ...
        for img_num in range(len(self.images)):
            ...
            for slice_num in tqdm(range(total_slices)):
                for k in self.labels:
                    ...
                    label_path = os.path.join(self.dest_path, split_path, k)
                    fname = (
                        f"{str(img_num)}_{pad_to_max_num(slice_num, total_slices)}.npy"
                    )
                    np.save(os.path.join(label_path, os.path.basename(fname)), sample)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Preprocessing script for oct dataset 2015"
    )
    ...
    if args.dataset == "UMN":
        processor = DataPreprocessorUMN(
            args.data_path, args.dest_path, slice_to_bscans, is_mat=args.is_mat
        )
